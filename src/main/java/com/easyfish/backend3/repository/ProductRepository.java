...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("select p from Product p where lower(trim(coalesce(p.category, ''))) = lower(trim(:category))")
    List<Product> findByCategoryNameNormalized(@Param("category") String category);

    @Query("select count(p) from Product p where lower(trim(coalesce(p.category, ''))) = lower(trim(:category))")
    long countByCategoryNameNormalized(@Param("category") String category);
}
