...
public class CartController {
    ...
    public CartController(CartItemRepository cartRepository, ProductRepository productRepository, CategoryRepository categoryRepository) {
    ...
    public List<Map<String, Object>> replaceUserCart(@PathVariable String userId, @RequestBody List<CartItemRequest> items) {
        ...
        if (items != null) {
            for (CartItemRequest request : items) {
                ...
                if (!isAvailableForNewOrder(product)) continue;
    ...
    private boolean isAvailableForNewOrder(Product product) {
        if (product == null || Boolean.FALSE.equals(product.getActive()) || Boolean.FALSE.equals(product.getPublished())) return false;
        String status = product.getStatus() == null ? "" : product.getStatus().trim().toUpperCase();
        if (status.equals("DRAFT") || status.equals("INACTIVE") || status.equals("DELETED") || status.equals("STOCK") || status.equals("STOCK_MASTER")) return false;
        if (product.getCategory() == null || product.getCategory().isBlank()) return false;
        return categoryRepository.findByNameIgnoreCase(product.getCategory().trim())
